...
run_full_uninstall() {
    ...
    echo "  -> Removing system files and user..."
    rm -rf "$PROJECT_DIR" \
           /etc/tinc \
...
change_configuration() {
    ...
    if [[ -n "$NEW_PORT" ]]; then
        echo "  -> Changing port..."
        # More robust command to get the old port number
    ...
    # --- Change Username and Password (FIXED) ---
    ...
    if [[ -n "$NEW_USER" || -n "$NEW_PASS" ]]; then
        ...
        # Improved command to capture and display output/errors from the python script
...
run_installation() {
    ...
    # --- 3. Setup Main Tinc Node ---
    echo -e "\n\033[1;34mStep 3/7: Configuring Tinc Main Node\033[0m"
    ...
    /usr/sbin/tincd -n "$TINC_NET_NAME" -K4096 &>/dev/null
    ...
    # --- 4. Setup Web Panel ---
    echo -e "\n\033[1;34mStep 4/7: Generating Web Panel Files & UI\033[0m"
    if ! id -u $APP_USER > /dev/null 2>&1; then
        useradd -r -m -s /bin/false $APP_USER
        echo "  -> Adding $APP_USER to 'systemd-journal' group for log access..."
        usermod -aG systemd-journal $APP_USER
    fi
    mkdir -p "$PROJECT_DIR/templates"
    chown -R $APP_USER:$APP_USER "/etc/tinc"
...
EOL
    ...
    echo "  -> Generated system permissions."
...
EOL
    ...
    # app.py (DEFINITIVE LOGIC WITH THREAD-SAFE DATA HANDLING + MINIMAL ADDITIONS FOR NEW UI)
...
def delete_node_task(task_id,node_id):
    ...
    try:
        ...
        os.remove(f"{hosts_dir}/{node_to_delete.name}"); os.remove(f"{clients_dir}/{node_to_delete.name}")
...
# --- START: New Routes for Main Server Actions ---
...
# --- END: New Routes for Main Server Actions ---
...
EOL
    ...
    # --- Generation of wsgi.py, initial_setup.py, and NEW update_credentials.py ---
...
EOL
    ...
    # --- START: NEW 2025-READY UI TEMPLATES ---
    # --- base.html (New Layout & Style) ---
...
<head>
    ...
    <style>
        ...
        .pulse-online { box-shadow: 0 0 0 0 rgba(16, 185, 129, 1); animation: pulse-green 2s infinite; }
        ...
        <nav class="bg-white shadow-md">
            <div class="container mx-auto px-6 py-3 flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    <svg class="h-8 w-8 text-slate-800" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path>
                        <path d="M12 11.1429C12.6219 11.1429 13.1429 10.6219 13.1429 10C13.1429 9.37813 12.6219 8.85714 12 8.85714C11.3781 8.85714 10.8571 9.37813 10.8571 10C10.8571 10.6219 11.3781 11.1429 12 11.1429Z"></path>
                        <path d="M12 12V14"></path>
                    </svg>
                    <h1 class="text-xl font-bold text-slate-800">DejTunnel</h1>
                </div>
                {% if session.logged_in %}
                <a href="{{ url_for('logout') }}" class="text-sm font-medium text-slate-600 hover:text-red-600 transition-colors">Logout</a>
                {% endif %}
            </div>
        </nav>
...
<div class="flex items-center justify-center min-h-[calc(100vh-200px)]">
    <div class="w-full max-w-sm p-8 space-y-6 bg-white rounded-2xl shadow-lg">
        <div class="text-center">
            <h2 class="text-2xl font-bold text-slate-900">DejTunnel Login</h2>
            <p class="mt-2 text-sm text-slate-600">Please sign in to continue</p>
        </div>
        <form method="POST" class="space-y-4">
            <div>
                <label for="username" class="sr-only">Username</label>
                <input type="text" name="username" id="username" placeholder="Username" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:outline-none transition" required>
            </div>
            <div>
                <label for="password" class="sr-only">Password</label>
                <input type="password" name="password" id="password" placeholder="Password" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:outline-none transition" required>
            </div>
            <button type="submit" class="w-full bg-slate-800 text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-900 transition-colors shadow-md">Sign In</button>
        </form>
    </div>
</div>
{% endblock %}
EOL
    ...
    # --- dashboard.html (COMPLETE UI OVERHAUL) ---
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1 space-y-8">
        <div class="bg-white p-6 rounded-2xl shadow-lg">
            <h2 class="text-xl font-bold text-slate-800 mb-4 border-b pb-3">Main Server Status</h2>
            {% if main_network %}
                ...
                <div class="flex justify-between items-center">
                    ...
                    {% if main_network.live_status.status == 'Online' %}
                        <div class="flex items-center space-x-2">
                            <div class="w-3 h-3 rounded-full bg-green-500 pulse-online"></div>
                            <span class="font-semibold text-green-600">Online</span>
                        </div>
                    {% else %}
                        <div class="flex items-center space-x-2">
                            <div class="w-3 h-3 rounded-full bg-red-500"></div>
                            <span class="font-semibold text-red-600">{{ main_network.live_status.status }}</span>
                        </div>
                    {% endif %}
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-500 font-medium">Node Name</span>
                    <span class="font-mono text-slate-900">{{ main_network.main_node_name }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-500 font-medium">Public IP</span>
                    <span class="font-mono text-slate-900">{{ main_network.main_public_ip }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-500 font-medium">Private IP</span>
                    <span class="font-mono text-slate-900">{{ main_network.main_private_ip }}</span>
                </div>
                 <div class="flex justify-between items-center">
                    <span class="text-slate-500 font-medium">Tinc Network</span>
                    <span class="font-mono text-slate-900">{{ main_network.net_name }}</span>
                </div>
            </div>
            <div class="mt-6 pt-6 border-t border-slate-200 flex space-x-2">
                <button onclick="viewMainLog('{{ main_network.main_node_name }}')" class="flex-1 text-sm bg-slate-100 text-slate-800 hover:bg-slate-200 font-semibold py-2 px-3 rounded-lg transition-colors">View Log</button>
                <form action="{{ url_for('restart_main_node') }}" method="POST" class="flex-1">
                     <button type="submit" onclick="return confirm('Are you sure you want to restart the main server Tinc service?')" class="w-full text-sm bg-blue-100 text-blue-800 hover:bg-blue-200 font-semibold py-2 px-3 rounded-lg transition-colors">Restart</button>
                </form>
            </div>
            {% else %}
            <p class="text-slate-500">Main server information not found.</p>
            {% endif %}
        </div>
    </div>
    ...
    <div class="lg:col-span-2">
        <div class="flex justify-between items-center mb-5">
            <h2 class="text-2xl font-bold text-slate-800">Remote Nodes</h2>
            <button onclick="showModal('addNodeModal')" class="bg-slate-800 hover:bg-slate-900 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors flex items-center space-x-2">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white p-5 rounded-2xl shadow-lg transition-shadow hover:shadow-xl">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                    <h3 class="text-xl font-bold text-slate-800 mb-2 sm:mb-0">{{ node.name }}</h3>
                    <div class="flex items-center space-x-2">
                        {% if node.live_status.status == 'Online' %}
                            <div class="w-3 h-3 rounded-full bg-green-500 pulse-online"></div><span class="text-sm font-medium text-green-600">Online</span>
                        {% else %}
                            <div class="w-3 h-3 rounded-full bg-red-500"></div><span class="text-sm font-medium text-red-600">Offline</span>
                        {% endif %}
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm mb-4">
                    <p><strong class="text-slate-500">Private IP:</strong> <span class="font-mono text-slate-700">{{ node.private_ip }}</span></p>
                    <p><strong class="text-slate-500">Public IP:</strong> <span class="font-mono text-slate-700">{{ node.public_ip }}</span></p>
                </div>
                <div class="mt-4 pt-4 border-t border-slate-200 flex flex-wrap gap-2">
                    <button onclick="viewLog({{ node.id }}, '{{ node.name }}')" class="flex-1 min-w-[80px] text-sm bg-slate-100 text-slate-800 hover:bg-slate-200 font-semibold py-2 px-3 rounded-lg transition-colors">View Log</button>
                    <form action="{{ url_for('restart_node', node_id=node.id) }}" method="POST" class="flex-1 min-w-[80px]">
                         <button type="submit" onclick="return confirm('Are you sure you want to restart node \'{{node.name}}\'?')" class="w-full text-sm bg-blue-100 text-blue-800 hover:bg-blue-200 font-semibold py-2 px-3 rounded-lg transition-colors">Restart</button>
                    </form>
                    <button onclick="deleteNode({{ node.id }}, '{{ node.name }}')" class="flex-1 min-w-[80px] text-sm bg-red-100 text-red-800 hover:bg-red-200 font-semibold py-2 px-3 rounded-lg transition-colors">Delete</button>
                </div>
            </div>
            {% else %}
            <div class="text-center py-12 px-6 bg-white rounded-2xl shadow-lg">
                <svg class="mx-auto h-12 w-12 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/></svg>
                <h3 class="mt-2 text-lg font-medium text-slate-900">No remote nodes found</h3>
                <p class="mt-1 text-sm text-slate-500">Get started by adding a new remote node.</p>
            </div>
            {% endfor %}
        </div>
    </div>
...
<div id="addNodeModal" class="modal fixed inset-0 bg-slate-900 bg-opacity-50 h-full w-full items-center justify-center z-50 p-4">
    <div class="p-8 w-full max-w-lg shadow-2xl rounded-2xl bg-white transform transition-all" onclick="event.stopPropagation()">
        ...
        <form id="addNodeForm" class="mt-6 space-y-4">
            ...
            <input class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:outline-none transition" type="password" name="ssh_pass" placeholder="SSH Password" required>
            <div class="flex justify-end pt-4 space-x-3">
                <button type="button" onclick="hideModal('addNodeModal')" class="px-5 py-2 bg-slate-200 text-slate-800 rounded-lg hover:bg-slate-300 transition-colors font-semibold">Cancel</button>
                <button type="submit" class="px-5 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 transition-colors font-semibold shadow-md">Add & Provision</button>
            </div>
        </form>
    </div>
</div>

<div id="logModal" class="modal fixed inset-0 bg-slate-900 bg-opacity-75 h-full w-full items-center justify-center z-50 p-4">
    <div class="p-6 w-full max-w-4xl shadow-2xl rounded-2xl bg-slate-900 text-white font-mono flex flex-col" onclick="event.stopPropagation()">
        <h3 id="logModalTitle" class="text-xl font-semibold mb-4 text-cyan-400"></h3>
        <pre id="logContent" class="flex-grow w-full h-96 overflow-y-scroll bg-black p-4 rounded-md text-sm whitespace-pre-wrap select-all"></pre>
        <div class="mt-4 text-center">
            <button id="closeLogBtn" class="px-8 py-2 bg-slate-600 rounded-md hover:bg-slate-700 transition-colors font-semibold">Close & Refresh</button>
        </div>
    </div>
</div>
...
<script>
    ...
    addNodeModal.addEventListener('click', () => hideModal('addNodeModal'));
    logModal.addEventListener('click', () => hideModal('logModal'));

    closeLogBtn.addEventListener('click', () => {
        hideModal('logModal');
        logContent.textContent = '';
        if (closeLogBtn.dataset.taskFinished === 'true') {
    ...
    async function runTask(endpoint, formData, title) {
        ...
        showModal('logModal');
        logContent.textContent = 'Starting task, please wait...\\n';
        ...
        try {
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    logContent.textContent = statusData.log.join('\\n');
                    logContent.scrollTop = logContent.scrollHeight;

                    if (statusData.status === 'Completed' || statusData.status === 'Failed') {
                        clearInterval(interval);
                        logContent.textContent += `\\n\\n--- TASK ${statusData.status.toUpperCase()} ---`;
                        logContent.textContent += `\\n\\nPanel will automatically refresh in 3 seconds...`;
                        setTimeout(() => {
                            hideModal('logModal');
                            window.location.reload();
                        }, 3000);
                    ...
                    clearInterval(interval);
                    logContent.textContent += `\\n\\n--- ERROR POLLING FOR STATUS: ${err.message} ---`;
                    closeLogBtn.dataset.taskFinished = 'true';
                }
            }, 2000);
        } catch (error) {
            logContent.textContent += `\\n\\n--- FATAL ERROR STARTING TASK: ${error.message} ---`;
            closeLogBtn.dataset.taskFinished = 'true';
        }
    }
    ...
    document.getElementById('addNodeForm').addEventListener('submit', function(e) {
        e.preventDefault();
        runTask("{{ url_for('start_add_node_task') }}", new FormData(this), 'Provisioning New Node...');
    });

    function deleteNode(nodeId, nodeName) {
        if (!confirm(`Are you sure you want to permanently delete node "${nodeName}"? This action cannot be undone.`)) return;
        runTask(`/api/delete_node_task/${nodeId}`, null, `Deleting Node: ${nodeName}...`);
    }
    ...
    async function fetchAndDisplayLog(endpoint, title) {
        ...
        try {
            ...
            logContent.textContent = `Failed to fetch logs: ${error.message}`;
        }
    }

    function viewLog(nodeId, nodeName) {
        fetchAndDisplayLog(`/api/get_remote_log/${nodeId}`, `Logs for Remote Node: ${nodeName}`);
    }

    function viewMainLog(nodeName) {
        fetchAndDisplayLog("{{ url_for('get_main_log') }}", `Logs for Main Server: ${nodeName}`);
    }
</script>
{% endblock %}
EOL
    echo "  -> Generated all new HTML templates for the 2025 UI."
    # --- END: NEW UI TEMPLATES ---

    # --- 5. Set Final Permissions & Python Env ---
    echo -e "\n\033[1;34mStep 5/7: Setting Ownership and Installing Python Packages\033[0m"
    chown -R $APP_USER:$APP_USER "$PROJECT_DIR"
    ...
    echo -e "\n\033[1;34mStep 6/7: Configuring and Starting System Services\033[0m"
...
Environment="PATH=$VENV_DIR/bin:/usr/bin:/bin:/sbin"
...
EOL
    ...
    # --- 7. Final DB Setup ---
    echo -e "\n\033[1;34mStep 7/7: Seeding Initial Database\033[0m"
